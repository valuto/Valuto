...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::VLU)
    {
    }
...
void OverviewPage::setBalance(const CAmount& balance, const CAmount& unconfirmedBalance, const CAmount& immatureBalance, const CAmount& anonymizedBalance, const CAmount& watchOnlyBalance, const CAmount& watchUnconfBalance, const CAmount& watchImmatureBalance)
{
    ...
    ui->label_VALUTO4->setVisible(showImmature || showWatchOnlyImmature);
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    if (model && model->getOptionsModel()) {
        ...
        connect(ui->blabel_VALUTO, SIGNAL(clicked()), this, SLOT(openMyAddresses()));

    }

    // update the display unit, to not use the default ("VALUTO")
    updateDisplayUnit();
}
...
void OverviewPage::updatBlockChainInfo()
{
    ...
    double BlockRewardVALUTO =  static_cast<double>(BlockReward)/static_cast<double>(COIN);

    ui->label_CurrentBlock_value->setText(QString::number(CurrentBlock));

    ui->label_CurrentBlockReward_value->setText(QString::number(BlockRewardVALUTO));
}
