...
bool ProcessBlockFound(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != chainActive.Tip()->GetBlockHash())
            return error("VALUTO Miner : generated block is stale");
    }
    ...
    if (!ProcessNewBlock(state, NULL, pblock))
        return error("VALUTO Miner : ProcessNewBlock, block not accepted");
...
{
    LogPrintf("VALUTO Miner started\n");
    ...
    //control the amount of times the client will check for mintable coins
        ...
        LogPrintf("Running VALUTO Miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
