...
/** Fees smaller than this (in uvaluto) are considered zero fee (for relaying and mining)
...
{
    if (nHeight == 1) 
        return 76772675 * COIN; // pre-mined amount as old Valuto minted total supply until December 12, 2020.

    // From December 2020 till year 2062
    else if (nHeight <= 43099200 && nHeight > 1) 
        return 0.75 * COIN;
    // From 2062 till year 2102
    else if (nHeight <= 85147200 && nHeight > 43099200) 
        return 0.375 * COIN;
    // From 2102 till year 2142 and later.
    else
        return 0.187  * COIN;
...
bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck, bool fAlreadyChecked)
{
    ...
    if ( ( ( pindex->nHeight >= 0 && pindex->nHeight <= 250) || (pindex->nHeight > 840136 && pindex->nHeight <= 840336) ) && block.IsProofOfStake())
        return state.DoS(100, error("ConnectBlock() : PoS period not active"),
            REJECT_INVALID, "PoS-early");

    if ( ( ( pindex->nHeight > 250 && pindex->nHeight <= 840136 ) || pindex->nHeight > 840336 ) && block.IsProofOfWork()) {
    ...
    if (block.IsProofOfStake()) {
        ...
        for (unsigned int i = 2; i < block.vtx.size(); i++)
            ...
            if(nHeight > 1) { // exclude premine
                ...
                CScript scriptDevPubKeyIn  = CScript() << Params().vALUTODevKey() << OP_CHECKSIG;
                CScript scriptFundPubKeyIn = CScript() << Params().vALUTOFundKey() << OP_CHECKSIG;
